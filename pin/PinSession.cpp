...
pin_Session::pin_Session()
{
	...
	this->credit_profile_flistp = NULL;
...
pin_Session::~pin_Session()
{
	...
	if(credit_profile_flistp) {
	   ...
	   PIN_FLIST_DESTROY(credit_profile_flistp, NULL)
	}
}
	...
	)
{
	...
	const char *opname = pcm_opcode_to_opname((int32) opcode);
	sprintf(buf,"Opcode %s input flist\0", opname );
	bool success = false;
	...
	if ( return_flistpp == NULL ) {
		...
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, 
				(char*)"Panic: call_opcode recieved a NULL flist pointer2pointer", ebufp );	
		return false;
	} 
//	else if ( *return_flistpp != NULL ){
	//	PIN_ERR_LOG_MSG(PIN_ERR_LEVEL_WARNING, 
	//			(char*)"call_opcode recieved a non-NULL *outputflist * ... " );	
//	}
	 ...
	 */
	sprintf(buf,"Opcode %s input flist\0", opname );
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, buf , inflistp );	

	if (this->pin_debug) 
	{
		cout << buf << endl;
		PIN_FLIST_PRINT(inflistp,NULL,localebufp);
	}

	PCM_OP (ctxp, opcode, flags, inflistp, return_flistpp, localebufp);

	if (return_flistpp && *return_flistpp) {
		sprintf(buf,"Opcode %s output flist\0", opname );
		PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, buf, *return_flistpp );	
		if (this->pin_debug) 
		{
			cout << buf << endl;
			PIN_FLIST_PRINT(*return_flistpp,NULL,localebufp);
		}
	}
	/*
	 * Check success / fail 
	 */
	success = (!PIN_ERR_IS_ERR (localebufp) );
	return (success);
}
...
bool pin_Session::readCreditProfileObj() {

    if(!this->isConnected()) return false;

    /** If already read, return here itself **/
    if(credit_profile_flistp != (pin_flist_t *)NULL) return true;

    pin_flist_t *crdt_search_flistp =NULL,*crdt_out_flistp=NULL;
    pin_flist_t *tmp_flistp=NULL;
    poid_t *crdt_search_pdp = NULL;
    poid_t *crdt_pdp = NULL;
    int32           s_flags = SRCH_DISTINCT;
    bool rc;
    const char *search_query = "select X from /config/credit_profile where F1 = V1 " ;
    PIN_ERR_CLEAR_ERR(ebufp);

    crdt_search_flistp = PIN_FLIST_CREATE(ebufp);
    crdt_search_pdp = PIN_POID_CREATE(this->getDBNumber(), (char*)"/search", (int64)-1, ebufp);
    PIN_FLIST_FLD_PUT(crdt_search_flistp, PIN_FLD_POID,(void *)crdt_search_pdp, ebufp);
    PIN_FLIST_FLD_SET(crdt_search_flistp, PIN_FLD_FLAGS, &s_flags, ebufp);
    PIN_FLIST_FLD_SET(crdt_search_flistp, PIN_FLD_TEMPLATE, (void*)search_query, ebufp);

    crdt_pdp = PIN_POID_CREATE(this->getDBNumber(), (char*)"/config/credit_profile", (int64)-1, ebufp);
    tmp_flistp = PIN_FLIST_ELEM_ADD(crdt_search_flistp, PIN_FLD_ARGS, 1, ebufp);
    PIN_FLIST_FLD_SET(tmp_flistp, PIN_FLD_POID, (void*)crdt_pdp, ebufp);

    tmp_flistp = PIN_FLIST_ELEM_ADD(crdt_search_flistp, PIN_FLD_RESULTS, 0, ebufp);
    PIN_FLIST_ELEM_SET(tmp_flistp, NULL, PIN_FLD_PROFILES, PCM_RECID_ALL, ebufp);

    rc = this->call_opcode( PCM_OP_SEARCH, 0, crdt_search_flistp, &crdt_out_flistp, ebufp);

    PIN_FLIST_DESTROY(crdt_search_flistp, NULL);
    credit_profile_flistp = (pin_flist_t *)NULL;
    credit_profile_flistp = (pin_flist_t *)PIN_FLIST_ELEM_TAKE(crdt_out_flistp, PIN_FLD_RESULTS, 0, 1,ebufp);

    if(crdt_out_flistp) PIN_FLIST_DESTROY(crdt_out_flistp, NULL);

    if(!rc || (credit_profile_flistp == (pin_flist_t *)NULL)) {
       return false;
    }
    return true;
}
